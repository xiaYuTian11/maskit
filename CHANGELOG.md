...
注入请求头占位符覆盖客户端真实凭据（中转站 401）修复版本。

### 修复
- 修复「注入请求头」仍留占位符时**把客户端自带的真实凭据覆盖掉**、导致上游返回 401「无效的令牌」：客户端类型选 Anthropic / OpenAI 时预设会预填 `x-api-key: <YOUR_API_KEY>` / `Authorization: Bearer <YOUR_API_KEY>`，用户未替换就保存后，转发阶段会无条件覆盖客户端发来的真 key（中转站场景必现，官方直连若客户端自带凭据同样中招）。现在整值为 `<...>` 的占位符一律跳过注入，客户端自带凭据照常透传，并在配置加载时告警一次。**已保存的旧配置无需手动清理即可恢复**。
- 修复注入请求头值为空时同样会覆盖客户端凭据的问题：空字符串不是凭据，注入等于把客户端的头清掉，比不注入更糟。现在空值一并跳过注入并告警。
- **「注入请求头」中的凭据类请求头一律不再注入**：Maskit 只配 URL、只做透明转发，凭据归客户端所有（Claude Code / Cursor 等都会自带），在这个字段里填凭据没有任何正当场景，只会把客户端自带的真凭据覆盖成配置里的值 —— 填错就是必然 401，而上游只报「无效的令牌」，用户完全看不出是自己配置造成的。覆盖范围：`Authorization`、`Proxy-Authorization`、`Cookie`、`X-Api-Key`、`Api-Key`、`Apikey`、`X-Goog-Api-Key`、`X-Auth-Token`、`X-Access-Token`、`X-Token`、`X-Session-Token`、`Private-Token`、`X-Gitlab-Token`、`X-Github-Token`、`X-Amz-Security-Token`、`X-Amz-Credential`、`X-Client-Secret`、`Client-Secret`。头名大小写不敏感、只做精确匹配（`x-api-key-version` 这类自造头不受影响）；与凭据无关的协议头（如 `anthropic-beta`）不受影响，照常注入。
- 修复客户端类型预设**预填凭据头占位符**这一配置陷阱：`Authorization` / `x-api-key` 不再进预设（客户端本来就会带自己的凭据，真需要注入时手填真实值即可），从源头消除「预设帮用户填了个假 key」。

### 优化
- 「注入请求头」的说明文案补全语义：原文案只写「客户端没带时在这里填上」，未说明同名头会覆盖客户端自带的凭据、也未说明一般应留空，是上述占位符误配的诱因之一。现明确该字段**只放与凭据无关的协议头**，凭据请配在客户端里。
- 「注入请求头」收进设置弹窗的「高级选项」折叠区，默认收起：请求头本来就原样透传（引擎只改写 `Host`，流式请求额外把 `accept-encoding` 置为 `identity`），不展开就绝不会注入任何头，避免用户误以为这是必填项。**已有配置（含历史遗留的占位符行）仍默认展开**，否则用户看不到问题行也删不掉。
- 设置页保存客户端时拦截占位符 header 值与凭据头并给出明确提示（凭据请配在客户端里 / 填入真实值或删除该行），历史配置里的凭据头还会在列表中标红，把故障挡在配置阶段而不是等上游回 401。**凭据头行为微调**：值为空时（用户在客户端内清空 Value 框而非点击删除按钮）放行保存，引擎侧本来就会跳过空值、设置页保存时也会把空行整行丢掉，放行后用户改完其它字段可以正常保存，不必强行删行。
- 凭据头列表标红范围同步：值为空的凭据头不再标红，与保存行为保持一致（标红 = 这行真的会被拦下），避免误导。

### 测试
- 补充「注入请求头为占位符/空值时不得覆盖客户端自带凭据」用例，覆盖 Bearer 形态、真实值仍覆盖、客户端未带照常注入、非占位符值不受影响、空值跳过、用户现场三行头原样复现共 7 个场景。
- 补充「凭据类请求头一律不得注入」用例，覆盖 10 个凭据头名、真实值也不注入、客户端未带也不补、头名大小写不敏感、子串不误伤（`x-api-key-version` / `x-token-count`）、协议头照常注入、空值一律跳过（与凭据无关）、占位符值一律跳过共 9 个场景。
- 补充「引擎与前端凭据头黑名单对齐」单测，通过 AST/正则解析前端 `Settings.tsx` 中的 `CREDENTIAL_HEADER_NAMES` 与引擎 `_CREDENTIAL_HEADER_NAMES` 校验 Set 等价性，防止后续迭代产生双名单漂移。
