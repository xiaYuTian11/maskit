...
| 版本检查 / 更新下载（桌面版经 Tauri Updater，Web / Docker 版由**服务端**探测 `latest.json` / GitHub Releases API） | **自动**：启动后约 8 秒静默查一次，之后每 6 小时复查；点「检查更新」可立即触发 | 当前版本号 | 无法关闭；仅探测版本号与元数据，不发送任何使用数据，下载仅在点击「安装」后发生 |

> Web / Docker 版的版本检查由**服务器**发起（`/api/update/check`），源顺序为
> 「自定义源（`update_check_url`）→ 静态 `latest.json` → GitHub API」，结果缓存 10 分钟。
> 出网方是服务器而非访问者的浏览器——国内/内网部署下浏览器到 GitHub 本就不通，
> 让浏览器直连只会得到 `TypeError: NetworkError`。静态资产不计入 GitHub 匿名 API 限流
> （60 次/小时/IP），缓存把多人共用出口的消耗压到 ~6 次/小时。
> 可用 `update_check_url` 指向镜像或自建中转（设置 → 关于与更新）。
